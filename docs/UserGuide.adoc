...
* Items in `UPPER_CASE` are the parameters to be entered by the user e.g. in `add n/NAME`, `NAME` is a parameter which can be used as `add n/John Doe`.
...
As you can see from Figure 4.13.1, it shows the bright theme, which is the default theme for Bevy.
After entering the command, the colour scheme will be changed to the dark theme as seen from Figure 4.12.2.
